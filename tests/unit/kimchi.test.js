...
describe("kimchi registry entry", () => {
  ...
  it("points at the OpenAI-compatible gateway with an authenticated UA", () => {
    ...
    expect(typeof ua === "string" && ua.length > 0).toBeTruthy();
  });
  ...
  it("exposes the upstream static models", () => {
    ...
    expect(ids.length >= 5).toBeTruthy();
  });
...
describe("kimchi oauth", () => {
  ...
  it("rejects a callback whose state does not match", async () => {
    await await expect(() => _handleCallback({ token: "castai_v1_x", state: "wrong" }, "expected")).rejects.toThrow();
  });
