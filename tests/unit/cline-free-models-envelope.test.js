...
describe("cline public catalog aggregation (resolveClineModels vs resolveClinepassModels)", () => {
  const API_MODELS_URL = "https://api.cline.bot/api/v1/ai/cline/models";
  ...
  const API_RESPONSE = [
    ...
    { id: "z-ai/glm-5.3-flash", name: "GLM-5.3 Flash", architecture: { modality: "text->text" } },
    { id: "z-ai/deepseek-v4-flash", name: "DeepSeek V4 Flash (Free)", architecture: { modality: "text->text" } },
  ];
  ...
  it("resolveClineModels keeps free text models and excludes ClinePass models", async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(API_MODELS_URL);
  ...
  it("resolveClinepassModels returns only cline-pass/ models", async () => {
    ...
    fetchMock.mockResolvedValue({
      ok: true,
      json: async () => ({ clinePass: API_RESPONSE }),
    });
    const result = await resolveClinepassModels({ accessToken: "test-token" });
    expect(result).not.toBeNull();
    expect(fetchMock.mock.calls[0][0]).toBe(RECOMMENDED_URL);
    expect(result.models).toHaveLength(2);
    const ids = result.models.map((m) => m.id);
  ...
  it("resolveClineModels permits public discovery without a token", async () => {
    const { resolveClineModels } = await import("../../open-sse/services/clinepassModels.js");
    fetchMock.mockResolvedValue({ ok: true, json: async () => ({ data: API_RESPONSE }) });
