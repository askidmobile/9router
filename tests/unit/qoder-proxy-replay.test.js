...
describe("Qoder signed inference transport", () => {
  ...
  it.each([true, false])("still supports successful inference with proxy=%s", async (useProxy) => {
    ...
    await result.response.text();
    expect(fetchMock).toHaveBeenCalledTimes(1);
    // The fork also uses a direct Agent for timeout/connection limits.
    expect(fetchMock.mock.calls[0][1].dispatcher.constructor.name).toBe(useProxy ? "ProxyAgent" : "Agent");
  });
