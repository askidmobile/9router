...
export async function ensureServiceSession(sess) {
  ...
  try {
    ...
    log(`exchanging passToken (region=${sess.region}, egress=${sess.proxyUrl ? "configured proxy" : "direct"}) ...`);
...
export async function proxyAccountRequest(sess, req, origin) {
  ...
  if (init.method !== "GET" && init.method !== "HEAD") {
    ...
    if (buf.length && /urlencoded|json|text/i.test(ctBody)) {
      ...
      if (after !== before) {
        ...
        console.log(`${new Date().toISOString().slice(11, 23)} [mimo-login] body STILL local, not matchable: ${before.length} bytes`);
      }
    }
    init.body = buf;
  }
  ...
  for (let hop = 0; hop < 8; hop++) {
    // Only Xiaomi cookies belong upstream. The browser cookie header contains
    // the 9router dashboard session and must never be used as a fallback.
    res = await fetchUpstream(sess, current, hop === 0 ? init : { ...init, body: undefined }, cookieHeaderFor(sess, current));
  ...
  return res.text().then((rawBody) => {
    ...
    if (/^\/(pass|sts)/.test(reqPath) || res.status >= 400) {
      ...
      if ((m && m[1] !== "0") || res.status >= 400) {
        console.log(`${new Date().toISOString().slice(11,23)} [mimo-login] upstream ${reqPath} http=${res.status} code=${m ? m[1] : "?"}`);
      }
    }
