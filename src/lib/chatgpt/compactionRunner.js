...
import { compactRequest, compactResponse, compactionTranscript, readCompactionCompletion } from "./compact.js";
...
export function compactionBudget(contextWindow) {
  ...
  const maxOutputTokens = Math.min(4096, Math.floor(context / 4));
...
async function summarizeParts(body, transcript, budget, invoke, signal) {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
      ...
      while (!failed && cursor < parts.length) {
        ...
        try {
          ...
          const request = compactRequest(body, {
            transcript: parts[index], maxOutputTokens: budget.maxOutputTokens,
            part: parts.length > 1 ? { index, count: parts.length } : undefined,
          });
          const response = await invoke(request, signal);
          summaries[index] = await readCompactionCompletion(response);
    ...
    signal.throwIfAborted();
    if (parts.length === 1) return Response.json(summaries[0].data);
        ...
        try {
          ...
          if (!response.ok) {
            ...
            const event = { type: "response.failed", response: { id: `resp_${randomUUID()}`, status: "failed", output: [], error: {
              code: "server_error", message: `Compaction failed (HTTP ${response.status}): ${message}. History was not replaced.`,
            } } };
