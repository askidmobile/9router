...
// Credential normalization must run after generic Bearer auth has been applied.
const AUTH_HEADER_HOOKS = {
  clineHeaders: (h, c) => Object.assign(h, buildClineCredentialHeaders(c)),
};
...
export class DefaultExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials, stream = true, url, model) {
    ...
    // Non-auth hooks run first so dynamic overlays cannot replace credentials.
    for (const hook of desc.hooks || []) HEADER_HOOKS[hook]?.(headers, credentials);
    applyAuth(headers, desc, credentials);
    for (const hook of desc.hooks || []) AUTH_HEADER_HOOKS[hook]?.(headers, credentials);
